...
- **Safety timeout:** declare **`scriptTimeout`** (**milliseconds**) in **`parseParams`** and register a **load-time** `setTimeout(..., scriptTimeout)` that checks **`cs.isClosed`**, **`cs.log`s** if the run overran, then **`scheduleClose()`**—see the scaffold **`src/index.ts`**. Tune the value per script in MI. In TypeScript, type the field as **`ScriptTimeout`** (imported from `@metricinsights/cs-helper`, structurally just `number`) instead of a bare `number`—this marks it for the build tooling regardless of the field's name; JS scripts use the **`@scriptTimeout <fieldName>`** JSDoc tag instead (next to `@type`).
