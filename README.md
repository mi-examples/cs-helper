...
| `--update-ai`               | Refresh AI assistant files in an existing project instead of scaffolding a new one (see below) | `false` |
...
#### Keeping AI assistant files up to date

Each ai-addon file scaffolded by `cs-helper-create` (e.g. `CLAUDE.md`, `.cursor/rules/metric-insights-custom-script.mdc`) embeds a hidden `cs-helper-addon-version` marker recording which cs-helper version generated it. When you run `npm run build` (`cs-helper <entry>`), the CLI checks that marker against the installed cs-helper version and prints a console warning if a file looks out of date—either because it was generated by an older version, or because it predates version tracking entirely.

To refresh the AI assistant files in an existing project without re-scaffolding, run `--update-ai` inside it:

```shell
# Refresh whichever AI assistant files are already present (auto-detected)
npx cs-helper-create --update-ai .

# Refresh (or add) specific tools
npx cs-helper-create --update-ai --ai claude --ai cursor .
```
...
- **`window.req?: string`** — present only when the custom script is invoked from an entity page request, and only if that entity page explicitly passes it through. Absent for scheduled runs, digests, and any entity page that doesn't pass it.
- **`window.user?: CustomScriptRequestUser`** — present only on Metric Insights instances **7.2.2 and later**. This is the user **making the request** through the custom script entity (e.g. viewing the entity page)—**not** the user configured to run the script. `CustomScriptRequestUser` (and `CustomScriptRequestUserGroup`) are exported from `@metricinsights/cs-helper` and describe the shape (`user_id`, `username`, `email`, `display_name`, `is_administrator`/`is_power_user`/`is_manage_pages_privilege` as `'Y' | 'N'`, `groups`, and an untyped `attributes: unknown[]`).
...
- **Betas.** Every push to `develop` with releasable commits publishes `X.Y.Z-beta.N` to npm under the `beta` dist-tag, with a GitHub prerelease. Install one with `npm install @metricinsights/cs-helper@beta`.
- **Production releases.**
