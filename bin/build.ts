...
/**
 * Extracts a human-readable URL from package.json repository field.
 * Supports both formats: string ("https://...") or object ({ type, url: "git+https://..." }).
 */
function parsePackageRepository(
    ...
    plugins: [
      new webpack.BannerPlugin({
        banner: function () {
          ...
          const helperRepository = parsePackageRepository(
...
Code sources:
  ...
  Package repository: ${parsePackageRepository(repository) || 'Not defined'}
...
***** ----- ----- ----- ----- ----- ----- *****${paramsDescription}${readme ? `\n\n***** README.md *****\n\n${readme}\n***** --------- *****` : ''}`;
  ...
  await new Promise<{
    ...
    if (err || stats?.hasErrors()) {
      ...
      const contentNormalized = contentWithoutTabs
        .replace(/\r\n/g, '\n')
        .replace(/\r/g, '\n');
